...
function ImageItem({ image, index, onDelete, isDragging }) {
  ...
  return (
    <div
      ...
      {...attributes}
      {...listeners}
      className="relative group rounded-lg overflow-hidden shadow-md transition-transform duration-200 hover:scale-105 cursor-grab"
    >
      <img
        src={image.url}
        alt={`Image ${image.id}`}
      ...
      <button
        ...
        className="absolute top-2 right-2 bg-red-500 text-white p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-200 z-10"
      >
        <svg
...
export default function App() {
  ...
  const [imageList, setImageList] = useState(existingImagesData);
  ...
  console.log("watchedImages are ", watchedImages);
  ...
  const handleDragEnd = (event) => {
    ...
    if (active.id !== over.id) {
      const oldIndex = imageList.findIndex((field) => field.id === active.id);
      const newIndex = imageList.findIndex((field) => field.id === over.id);

      // Update local state for smooth animation
      setImageList((prevImages) => arrayMove(prevImages, oldIndex, newIndex));

      // Sync the form state with the new order
  ...
  const handleConfirmDelete = () => {
    const image = imageList.find((img) => img.id === imageToDelete);
    if (image && image.type === "existing") {
  ...
  const activeImage = activeId
    ? imageList.find((image) => image.id === activeId)
    : null;
