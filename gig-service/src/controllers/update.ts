...
import { BadRequestError, ISellerGig, isDataURL, uploads } from '@fvoid/shared-lib';
...
const gigUpdate = async (req: Request, res: Response): Promise<void> => {
  ...
  const updatedGig: ISellerGig|null = await updateGig(req.params.gigId, gig);
...
const gigUpdateActive = async (req: Request, res: Response): Promise<void> => {
  const updatedGig: ISellerGig |null = await updateActiveGigProp(req.params.gigId, req.body.active);
  res.status(StatusCodes.OK).json({ message: 'Gig updated successfully.', gig: updatedGig });
};
