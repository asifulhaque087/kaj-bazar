...
export class ReviewSeededListener extends FanoutListener<ReviewSeeded> {
  ...
  queueName: ReviewSeededQueues = "review-seeded-gig-queue";
  ...
  async onMessage(data: ReviewSeeded["data"], message: ConsumeMessage) {
    ...
    for (let i = 0; i < reviews.length; i++) {
      ...
      const { ratings: newRating, buyerIsSender, gigId } = review;

      if (buyerIsSender) continue;

      const Table = GigsTable;

      await updateEntityRatings(Table, gigId, newRating);
    }
